...
public class SendRequest {
    ...
    public static SendRequest emptyWalletMessage(Address destination, Coin value, String message) {
        SendRequest req = new SendRequest();
        final NetworkParameters parameters = destination.getParameters();
        checkNotNull(parameters, "Address is for an unknown network");
        req.tx = new Transaction(parameters);
        String op_return_message=message;
        req.tx.addOutput(Coin.ZERO, new ScriptBuilder().op(ScriptOpCodes.OP_RETURN).data(op_return_message.getBytes()).build());
        req.tx.addOutput(value, destination);
        //req.emptyWallet = true;
        return req;
    }
    public static SendRequest sendMessage(Address destination, Coin value, String message) {
        SendRequest req = new SendRequest();
        final NetworkParameters parameters = destination.getParameters();
        checkNotNull(parameters, "Address is for an unknown network");
        req.tx = new Transaction(parameters);
        String op_return_message=message;
        System.out.println(op_return_message);
        req.tx.addOutput(Coin.ZERO, new ScriptBuilder().op(ScriptOpCodes.OP_RETURN).data(op_return_message.getBytes()).build());
        req.tx.addOutput(value, destination);
