...
public abstract class AbstractBlockChain {
    ...
    public static BigInteger proofOfWorkLimit = Utils.decodeCompactBits(0x1e08ffffL);
    ...
    private void checkDifficultyTransitions_V2(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        final long      	BlocksTargetSpacing			= 120; // 2.5 minutes
        int         		TimeDaySeconds				= 60 * 60 * 24;
        long				PastSecondsMin				= TimeDaySeconds / 4;  //6 hours
        long				PastSecondsMax				= TimeDaySeconds * 7;  // 7 days
        long				PastBlocksMin				= PastSecondsMin / BlocksTargetSpacing;  //144 blocks --> 180
        long				PastBlocksMax				= PastSecondsMax / BlocksTargetSpacing;  //4032 blocks --> 5040
	...
	/* current difficulty formula, vertcoin - kimoto gravity well */
        ...
        { 
        	...
        	PastDifficultyAverage = proofOfWorkLimit;
    ...
    {
        if (calcDiff.compareTo(proofOfWorkLimit) > 0) {
            log.info("Difficulty hit proof of work limit: {}", calcDiff.toString(16));
            calcDiff = proofOfWorkLimit;
        }
