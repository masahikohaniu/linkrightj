...
public class Peer extends PeerSocketHandler {
    ...
    // Indicate whether downloading block for op_return message
    // Masahiko
    private volatile boolean vDownloadOpReturn;
    ...
    protected void processMessage(Message m) throws Exception {
        ...
        if (m instanceof Ping) {
        	...
        	//log.info("Masahiko - Peers. InventoryMessage" + m.toString());
            processInv((InventoryMessage) m);
        } else if (m instanceof Block) {
        	//log.info("Masahiko - Peers. Block" + m.toString());
            processBlock((Block) m);
        } else if (m instanceof FilteredBlock) {
        	//log.info("Masahiko - Peers. FilteredBlock" + m.toString());
            startFilteredBlock((FilteredBlock) m);
        } else if (m instanceof Transaction) {
        	//log.info("Masahiko - Peers. Transaction" + m.toString());
            processTransaction((Transaction) m);
        } else if (m instanceof GetDataMessage) {
        	//log.info("Masahiko - Peers. GetDataMessage" + m.toString());
    ...
    protected void processUTXOMessage(UTXOsMessage m) {
        ...
        lock.lock();
        log.info("Masahiko - Peers.processHeaders");
        try {
    ...
    protected void processGetData(GetDataMessage getdata) {
    	log.info("Masahiko - Peers.processGetData");
    ...
    protected void processTransaction(final Transaction tx) throws VerificationException {
        ...
        tx.verify();
        lock.lock();
        log.info("Masahiko - Peers.processTransaction");
        try {
    ...
    protected void processBlock(Block m) {
        ...
        if (vDownloadData) processBlockOpReturn(m);
    					...
    					for (ScriptChunk chunk:script.getChunks()) { 
    						...
    						if (chunk.isPushData()) {
    							...
    							for (String hashtag : hashtags) {
    								BTreeMap btmap = db.treeMap(hashtag).createOrOpen();
    ...
    public String getOpReturn() {    	        
        StoredBlock chainHead = blockChain.getChainHead();
        //int tips=chainHead.getHeight();        
        chainHeadHash=chainHead.getHeader().getHash();        
        blockcount=0;
