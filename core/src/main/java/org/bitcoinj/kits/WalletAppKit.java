...
public class WalletAppKit extends AbstractIdleService {
    ...
    protected DB db;
    public static final String APP_DB = "LinkRightMessage.db";
    ...
    protected void startUp() throws Exception {
        ...
        try {
            ...
            log.info("Before createOrLoadWallet(shouldReplayWallet)");
            vWallet = createOrLoadWallet(shouldReplayWallet);
            log.info("After createOrLoadWallet(shouldReplayWallet)");
        	...
        	log.warn("error on accessing file");
    ...
    private Wallet createOrLoadWallet(boolean shouldReplayWallet) throws Exception {
        ...
        log.info("after maybeMoveOldWalletOutOfTheWay()");
        if (vWalletFile.exists()) {
            wallet = loadWallet(shouldReplayWallet);
        } else {
        	
        	log.info("before createWallet()");
            wallet = createWallet();
            log.info("after createWallet()");
    ...
    protected Wallet createWallet() {
        ...
        if (walletFactory != null) {
        	...
        	/*        	
        	String pubkey;
        	DeterministicKey deterkey;
        	Wallet newWallet;
        	System.out.println("before do loop");  
        	do {        		
        		kcg = new KeyChainGroup(params);        		
        		deterkey=kcg.currentKey(KeyChain.KeyPurpose.RECEIVE_FUNDS);        		
        		pubkey = kcg.currentAddress(KeyChain.KeyPurpose.RECEIVE_FUNDS).toString();
        	//}while(!pubkey.substring(0,5).toLowerCase().equals("right"));
        	}while(!pubkey.substring(0,4).equals("Rmsg"));
        	newWallet=new Wallet(params, kcg);        	              
            return newWallet;  
            */
    ...
    private void maybeMoveOldWalletOutOfTheWay() {
        ...
        if (!vWalletFile.renameTo(newName)) {
            // This should not happen unless something is really messed udb.close();p.
